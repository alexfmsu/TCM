...
def run_RWA(w0, H, t0, t1, initstate, certain_state, nt = 200, ymin=0, ymax=1, RWA=True, title='title', color='blue'):
	...
	#------------------------------------------------------------------------------------------------------------------
	animator.make_plot(t, t0, t1, ymin, ymax, w[:,st+at], color, title=title, X=r'$t,\ мкс$', Y=r'$Probability$   ')
...
def run_RWAw(w0, H, t0, t1, initstate, certain_state1, certain_state2, nt = 200, ymin=0, ymax=1, RWA=True, title='title', color='blue'):
	...
	# ------------------------------------------------------------------------------------------------------------------
	animator.make_plot2w(t, t0, t1, ymin, ymax, w[:,st1+at1], w[:,st2+at2], initstate, st1=stt1, st2=stt2, title=title, X=r'$t,\ мкс$', Y=r'$Probability$   ')
...
def run2(w0, H_RWA, H_EXACT, t0, t1, initstate, certain_state, nt = 200, ymin=0, ymax=1, title='title'):
	...
	for i in range(0, at_count):
		at+=pow(2, i) * certain_state[1][at_count-i-1]
	#------------------------------------------------------------------------------------------------------------------
	animator.make_plot2(t, t0, t1, ymin, ymax, w_RWA[:,st+at], w_EXACT[:,st+at], title, X=r'$t,\ мкс$', Y=r'$Probability$   ')
...
def diff(obj):
    ...
    title += r'$w_c =  ' + str(wc) + '\ МГц,$   ' + r'$w_a = ' + str(wa) + '\ МГц,$   ' + r'$g = ' + str(g) + '\ c^{-1}$'
